...
(function () {
    ...
    const DEFINITIONS = [
        ...
        { id: 'rhythm_gold',    icon: '🥁', title: 'Metronome-proof',  desc: 'Earn gold in rhythm training.' },
    ...
    const TESTS = {
        ...
        rhythm_gold:    c => hasGold(c.medals, k => k.indexOf('rhythm:') === 0),
        // Bias AND spread both have to be tiny: averaging zero by scattering
        // equally early and late is not being in the pocket.
        in_the_pocket:  c => c.sessionType === 'rhythm' && c.result
                             && c.result.correct >= 8
                             && Math.abs(c.result.meanError) <= 5
                             && c.result.deviation <= 15,
        internal_clock: c => c.sessionType === 'rhythm' && !!c.gapDrill
    ...
    // Title and description are translated in place, once, with the English
    // text above as the fallback (which is what the Node tests, where no T
    // exists, always get). Rewriting the table rather than copying keeps
    // DEFINITIONS and the objects handed back the same identities.
    const tr = (key, fallback) => (typeof T === 'function' ? T(key, null, fallback) : fallback);
    let localised = false;
    function localiseDefinitions() {
        if (localised || typeof T !== 'function') return;
        localised = true;
        DEFINITIONS.forEach(def => {
            def.title = tr('achievement.' + def.id + '.title', def.title);
            def.desc = tr('achievement.' + def.id + '.desc', def.desc);
        });
    }
