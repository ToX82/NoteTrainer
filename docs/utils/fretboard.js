...
(function () {
    ...
    function create(container) {
        ...
        function noteLabel(midi) {
            if (!M) return '';
            // Compact form: the label sits in ~70px next to the nut.
            return M.shortNameOf(M.pitchClass(midi), false) + M.octaveOf(midi);
        }
        ...
        function markDetected(midi) {
            ...
            for (let i = 0; i < stringCount; i++) {
                M.fretsForPitchClassOnString(openMidi[i], pc, maxFret).forEach(f => {
                    _dot(i, f, 'is-detected', M.shortNameOf(pc, false));
                });
            }
        }
